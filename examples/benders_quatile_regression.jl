...
# We apply Norm-1 regression to the
...
# application. Every time you find a IF block with the flag `PARAM`
...
# can be represented by a set of supporting hyper-planes:
#
# $$
#\begin{align}
#  z_k(x) \ = \ & \min_{z, x}       &&  z && \notag \\
...
# $$
#
# But now it is only an underestimated problem.
